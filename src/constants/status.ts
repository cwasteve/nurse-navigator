...
export const STATUS_CONFIG: Record<MatchStatus, StatusMeta> = {
  [STATUS.UNREVIEWED]: { label: 'Unreviewed', borderColor: 'border-l-transparent' },
...
export const TABS: TabDef[] = (Object.entries(STATUS_CONFIG) as [MatchStatus, StatusMeta][]).map(([key, meta]) => ({
  key,
  label: meta.label,
}));
